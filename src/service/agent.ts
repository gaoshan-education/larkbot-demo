...
export class AgentService {
  ...
  async generateResponse(chatId: string, query: string) {
    ...
    try {
      ...
      // 最多重试 3 次以等待远程 MCP 工具加载
      // Retry up to 3 times to wait remote MCP tools becoming available
      for (let round = 1; round <= 3; round++) {
        ...
        for (const mcpClient of userContext.mcpClients) {
          try {
            ...
            console.warn('[Agent] 获取 MCP 工具失败 / Failed to get MCP tools:', e);
          }
        }
        if (Object.keys(tools).length > 0) {
          break;
        }
        console.log(`[Agent] 第 ${round} 轮未获取到任何工具，等待后重试 / No tools in round ${round}, retrying...`);
      ...
      console.log(
        ...
        Object.keys(tools),
        '是否包含 tavily_search ->',
        Object.prototype.hasOwnProperty.call(tools, 'tavily_search')
      );
      if (!Object.keys(tools).length) {
        console.warn('[Agent] 未获取到任何 MCP 工具，模型将无法进行工具调用 / No MCP tools acquired, model cannot call tools');
      }
