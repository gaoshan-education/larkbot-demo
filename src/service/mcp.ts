...
export class MCPClientService {
  ...
  /**
   * 创建 Tavily 搜索 MCP 客户端 (通过本地 stdio 服务器)
   * Create Tavily search MCP client via local stdio server
   * @returns Promise<MCPClient>
   */
  static async createTavilyMCPClient() {
    let command = 'npx';
    let args = ['-y',
                "mcp-remote",
                "https://mcp.tavily.com/mcp/?tavilyApiKey=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",
  ];
    if (process.platform === 'win32') {
      args = ['/c', command, ...args];
      command = 'cmd.exe';
  ...
  /**
   * 创建 MSlearn 搜索 MCP 客户端 (通过本地 stdio 服务器)
   * Create MSlearn search MCP client via local stdio server
   * @returns Promise<MCPClient>
   */
  static async createMSlearnMCPClient() {
    let command = 'npx';
    let args = ['-y',
                "mcp-remote",
